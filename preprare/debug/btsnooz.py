#!/usr/bin/env python
# Copy from https://cs.android.com/android/platform/superproject/+/master:packages/modules/Bluetooth/system/tools/scripts/btsnooz.py
"""
...
def main():
    ...
    try:
        ...
        p = subprocess.Popen(
            ["egrep", "-a", "BTSNOOP_LOG_SUMMARY", sys.argv[1]], stdout=subprocess.PIPE)
        p.wait()

        if (p.returncode == 0):
            sys.stderr.write(
                'Failed to parse uuencoded btsnooz data from bugreport.\n')
